...
path_config() {
    echo
    while :; do
        echo -e "Please enter the path you want ${magenta} to use to divert $none , such as /example , then just enter example ."
        read -p "$(echo -e "(default: [${cyan}example$none]):")" path
        [[ -z $path ]] && path="example"
...
uninstall() {
    ...
    if [[ -f /usr/bin/v2ray/v2ray && -f /etc/v2ray/config.json ]] && [[ -f $backup && -d /etc/v2ray/233boy/v2ray ]]; then
		...
		Remarks...Only supports uninstalling and using the V2Ray one-click installation script provided by me (example.com)
		" && exit 1
    fi
...
while :; do
    echo
    echo ".......... V2Ray One-Click Installation Script & Admin Script ..........."
    echo
    echo
    echo " 1. Install"
    echo
